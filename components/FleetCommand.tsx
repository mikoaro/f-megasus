...
interface FleetCommandProps {
    ...
    critical: boolean;
    userId: string;
    demoMode: "HUMAN" | "AI_AUTO"; // New Prop
}

export function FleetCommand({ telemetry, agentDecision, agentStatus, critical, userId, demoMode }: FleetCommandProps) {
    ...
    const isAuto = demoMode === "AI_AUTO";
    const headerColor = isAuto ? "text-purple-400" : "text-blue-400";
    const headerBg = isAuto ? "bg-purple-500" : "bg-blue-500";
    const headerTitle = isAuto ? "AUTONOMOUS SWARM" : "HUMAN OVERSIGHT";
        ...
        <div className="flex flex-col gap-6 h-full font-mono text-xs overflow-y-auto">
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-5 space-y-6 flex flex-col">
                    {/* AGENT TERMINAL */}
                    <div className={`bg-black/40 border ${isAuto ? 'border-purple-500/30' : 'border-blue-500/30'} rounded-xl p-6 flex-1 flex flex-col transition-colors duration-500`}>
                        <div className="flex justify-between items-center mb-4">
                            <h2 className={`text-sm ${headerColor} font-bold tracking-widest flex items-center gap-2`}>
                                <span className={`w-2 h-2 ${headerBg} rounded-full`}></span>
                                AI COMMAND LINK: {headerTitle}
                            </h2>
                            {critical && <span className="text-[10px] bg-red-500 text-black px-2 py-0.5 rounded font-bold animate-pulse">ACTION REQUIRED</span>}
                        </div>
                        
                        <div className="mb-4">
                            <AgentTerminal decision={agentDecision} statusStream={agentStatus} />
                        </div>
                        ...
                        <div className="mt-auto">
                            {(critical || agentDecision) ? (
                                ...
                                <div className="h-12 flex items-center justify-center border border-white/5 rounded bg-white/5 text-slate-600 text-xs italic">
                                    {isAuto ? "Swarm Agents in Standby..." : "Marathon Agent in Standby..."}
                                </div>
                            )}
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-3 gap-6 h-64">
                ...
                <div className="flex flex-col bg-black/40 border border-white/10 rounded-xl overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-2">
                        {sorTickets.map((t, idx) => (
                            <div key={t.ticket_ref || idx} className="bg-slate-900/80 border-l-2 border-red-500 p-3 rounded">
                                <div className="flex justify-between mb-1">
                ...
                <div className="flex flex-col bg-black/40 border border-white/10 rounded-xl overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-2">
                        {sorInventory.map((i, idx) => (
                            <div key={i.part_number || idx} className={`bg-slate-900/80 border-l-2 p-3 rounded flex justify-between items-center ${i.stock_level > 0 ? 'border-purple-500' : 'border-slate-600 opacity-50'}`}>
                                <div>
                ...
                <div className="flex flex-col bg-black/40 border border-white/10 rounded-xl overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-2">
                        {sorDispatches.map((d, idx) => (
                            <div key={d.work_order_ref || idx} className="bg-slate-900/80 border-l-2 border-green-500 p-3 rounded">
                                <div className="flex justify-between mb-1">
